...
app.get("/products/:id", async (req, res, next) => { // These normal route handlers also have access to next call.
  const { id } = req.params;
  ...
  if (!product) {
    return next(new AppError("Product Not Found", 404)); // We are returning cause i don't want the res.render down below to run after the this call is executed if return is not written.
  }
