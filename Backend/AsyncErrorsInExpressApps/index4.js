// First copying index3.js
// In this file we will see that there is a property called name that is associated with every mongoose error and depending on whether it is a validation error or cast error you can implement different responses for them.
// Also one more thing to note only the mongoose errors have a name property not all errors will have that.
...
const wrapAsync = (fn) => {
  // This uses the property of async functions returning promises since the function passed in inside is an async function that means it will return a promise so i can chain a .catch() method to that function.
...
// Setting up insert route (no need to add error handling here as it is not a async function and these non async function's errors can easily be handled using default express error handler)
...
const handleValidationError = (err) => {
  // console.dir(err);
  // In a real app, we would do a lot more here...like you might want to make the missing/incorrect field red or something and ask for that input again.
  return new AppError(`You Have A Validation Error...${err.message}`, 400);
};

// Setting up a custom error handler for different kinds of mongoose errors.
